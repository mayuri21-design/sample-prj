    ...
    if ($result->num_rows>0) {
        while ($row = $result->fetch_assoc()) {
            if ($row["Status"]=="Accepted") {
                $r = array('AppointmentDate'=>$row["AppointmentDate"],'Title'=>$row["Title"],'Description'=>$row["Description"],'FromTime'=>$row["FromTime"],'ToTime'=>$row["ToTime"],'events'=>$row["events"],'eventlevel'=>$row["eventlevel"],'act'=>$row["act"]);

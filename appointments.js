	...
	xmlhttp.onreadystatechange = function(){
	    if(this.readyState==4&&this.status==200){
	    	...
	    	for(i=0;i<data.length;i++){
	    		createAppBox(data[i].Title,data[i].Description,data[i].FromTime,data[i].ToTime,data[i].events,data[i].eventlevel,data[i].act);
	    	}	
	    }
	};
...
function addAppointment(){
	...
	// add code
	var events=document.getElementById("cat1").value;
	var eventlevel=document.getElementById("cat2").value;
	var act=document.getElementById("cat3").value;

	//end code


	createAppBox(title,desc,from,to,events,eventlevel,act);
	addAppointmentDb(appDate,title,desc,from,to,events,eventlevel,act);
...
function addAppointmentDb(appDate,title,desc,from,to,events,eventlevel,act){
	...
	var purpose = "add";
	var params = "appDate="+appDate+"&title="+title+"&description="+desc+"&appFrom="+from+"&appTo="+to+"&events="+events+"&eventlevel="+eventlevel+"&act="+act+"&inviteeStr="+inviteeStr+"&purpose="+purpose;
	xmlhttp.onreadystatechange = function(){
	    if(this.readyState==4&&this.status==200){
...
function createAppBox(title,desc,from,to,events,eventlevel,act){
	...
	var eventleveldiv=document.createElement("div");
	...
	var eventsText=document.createTextNode("Event Name :"+events);
	var eventlevelText=document.createTextNode("Event Level :"+eventlevel);
	var actText=document.createTextNode("Activity :"+act);
	...
	eventsdiv.appendChild(eventsText);
	eventleveldiv.appendChild(eventlevelText);
	...
	bodyDiv.appendChild(eventsdiv);
	bodyDiv.appendChild(eventleveldiv);
	timingsDiv.appendChild(actdiv);
	...
	//add code
	eventsdiv.setAttribute("class","tdyEventClass");
	eventleveldiv.setAttribute("class","tdyEventClass");
	actdiv.setAttribute("class","tdyEventClass");
